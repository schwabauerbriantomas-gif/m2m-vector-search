...
def chart_benchmark_comparison():
    ...
    ax1.set_title(
        f"Latencia de Busqueda ({n_splats:,} vectores)", fontweight="bold", pad=15
    )
...
def chart_architecture():
    ...
    for tier, x, color in tiers:
        rect = mpatches.FancyBboxPatch(
            (x - 1, 1.3), 2, 1.8, boxstyle="round,pad=0.05", facecolor=color, alpha=0.9
        )
        ax.add_patch(rect)
        ax.text(
            x,
...
def chart_data_types():
    ...
    bars = ax.barh(
        categories, m2m_works, color=colors, edgecolor="#333", linewidth=1.5, height=0.6
    )
    ...
    # Linea de decision
    ax.axvline(
        x=0.5, color="#333", linestyle="--", linewidth=2, label="Umbral de decision"
    )
