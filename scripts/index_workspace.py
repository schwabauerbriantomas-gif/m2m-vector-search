#!/usr/bin/env python3
"""
index_workspace.py — Index workspace files into SemanticMemoryDB

Indexes all .md, .py, .json files from a specified workspace directory.

Usage:
    python index_workspace.py [--workspace-dir /path/to/workspace] [--storage-path ./indexed_memory] [--reindex]
...
M2M_SRC = os.environ.get("M2M_SRC", os.path.join(os.path.dirname(__file__), "..", "src"))
if M2M_SRC not in sys.path:
    sys.path.insert(0, M2M_SRC)

WORKSPACE = Path(os.environ.get("WORKSPACE_DIR", Path.home() / "workspace"))
M2M_PROJECT = Path(os.path.dirname(__file__), "..").resolve()
...
def index_workspace(
    ...
    """
    Index the entire workspace into SemanticMemoryDB.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Index workspace files into SemanticMemoryDB")
    parser.add_argument("--storage-path", default="./semantic_indexed_memory", help="Storage path for the index")
