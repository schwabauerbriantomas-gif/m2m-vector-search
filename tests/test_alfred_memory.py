"""Tests for AlfredMemoryDB — Alfred's semantic memory interface."""
...
from m2m import AlfredMemoryDB, M2MConfig
...
def memory_db(encoder):
    return AlfredMemoryDB(encoder=encoder, latent_dim=384)


class TestAlfredMemoryDB:
    """Core AlfredMemoryDB tests."""
    ...
    def test_no_encoder_raises(self):
        ...
        db = AlfredMemoryDB(encoder=None, latent_dim=384)
...
class TestBM25Index:
    ...
    def test_unicode_tokenization(self):
        from m2m.bm25_index import BM25Index
        bm25 = BM25Index()
        bm25.add("d1", "Señor Schwabauer está aquí")
        results = bm25.search("schwabauer", k=1)
        assert len(results) == 1
