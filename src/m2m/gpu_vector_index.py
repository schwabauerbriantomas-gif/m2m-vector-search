...
Soporta dos backends GPU:
  - Vulkan: Compute shaders nativos (requiere SDK Vulkan + glslc)
  - CUDA:   PyTorch CUDA tensors (requiere torch + GPU NVIDIA)

Auto-detección: si CUDA está disponible y se solicita, usa CUDA.
...
def _has_cuda() -> bool:
    """Verifica si CUDA está disponible vía PyTorch."""
    try:
        import torch
...
def _has_vulkan() -> bool:
    """Verifica si Vulkan está disponible."""
    try:
...
class CUDAVectorIndex:
    """
    GPU vector index usando PyTorch CUDA tensors.

    API compatible con GPUVectorIndex pero usa CUDA en vez de Vulkan.
    Requiere: torch + GPU NVIDIA.
    """
    ...
    def __init__(self, index_vectors: np.ndarray, max_batch_size: int = 100):
        ...
        max_rows = free_mem // (bytes_per_row * 2)
        self._chunk_size = min(32768, self._n, max(1, max_rows))

        print(
            f"[CUDAVectorIndex] Ready -- index {self._n:,}x{self._dim}  "
            f"({index_np.nbytes / 1024**2:.1f} MB on GPU)  "
            f"chunk_size={self._chunk_size:,}"
        )
    ...
    def batch_search(self, queries: np.ndarray, k: int = 64):
        ...
        for chunk_start in range(0, self._n, chunk):
            chunk_end = min(chunk_start + chunk, self._n)
            chunk_tensor = self._index_tensor[chunk_start:chunk_end]

            q_sq = (query_tensor**2).sum(dim=1, keepdim=True)
            x_sq = (chunk_tensor**2).sum(dim=1, keepdim=True).T
            dot = query_tensor @ chunk_tensor.T
            dists_sq = q_sq + x_sq - 2 * dot
            dists_sq = torch.clamp(dists_sq, min=0.0)
            chunk_dists = torch.sqrt(dists_sq).cpu().numpy()
            ...
            combined_dists = np.concatenate([best_dists, chunk_dists], axis=1)
    ...
    def compute_distances(self, query_np, expert_embeddings_np):
        ...
        e = torch.from_numpy(expert_embeddings_np).to(self._device)
        dists = torch.cdist(q, e).squeeze(0)
        return dists.cpu().numpy()


# ═══════════════════════════════════════════════════════════════════════════════
# Factory — auto-detect best GPU backend
# ═══════════════════════════════════════════════════════════════════════════════
...
def create_gpu_index(index_vectors: np.ndarray, max_batch_size: int = 100,
    ...
    Returns GPUVectorIndex, CUDAVectorIndex, o None (CPU fallback).
    """
    if prefer_cuda and _has_cuda():
