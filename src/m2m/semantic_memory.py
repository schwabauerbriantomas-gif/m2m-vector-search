...
Convenience wrapper around M2M Vector Search designed for
AI agent semantic memory use cases.
...
Usage:
    ...
    >>> mem = SemanticMemoryDB(storage_path="./semantic_memory")
    >>> mem.store("The team decided to use M2M for semantic memory",
...
class SemanticMemoryDB:
    ...
    Example:
        ...
        >>> mem = SemanticMemoryDB(storage_path="./mem", encoder=my_encoder,
    ...
    def __init__(
        ...
        # M2M Vector DB (disable LSH fallback - agent memories are diverse)
    ...
    def _encode(self, text: Union[str, List[str]]) -> np.ndarray:
        ...
        if self.encoder is None:
            raise RuntimeError(
                "No encoder set. Provide an encoder callable to SemanticMemoryDB, "
