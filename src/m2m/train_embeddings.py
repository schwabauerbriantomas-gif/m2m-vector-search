...
# Local imports
try:
    from m2m.embedding_config import DatasetConfig, EmbeddingConfig
...
class SyntheticEmbeddingDataset(Dataset):
    ...
    def __init__(
        ...
        assert (
            self.teacher_embeddings.shape[1] == 384
...
def train_one_epoch(
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        with autocast(enabled=scaler.is_enabled()):
            ...
            teacher_emb = get_teacher_embeddings(model, batch["input_ids"], batch["attention_mask"])
...
def main():
    ...
    for m in all_metrics:
        log.info(
            f"Epoch {m['epoch']}: loss={m.get('epoch_loss', 0):.4f}, "
