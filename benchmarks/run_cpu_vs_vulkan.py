...
def run_benchmark(device_name, n_splats=10000, n_queries=100, k=64, latent_dim=640):
    ...
    print(
        f"[CONFIG] device={config.device}, compute_device={config.compute_device}, vulkan={config.enable_vulkan}"
    )
    ...
    print(
        f"[ADD] Added {n_added} splats in {add_time:.3f}s ({n_added/add_time:.0f} splats/s)"
    )
...
def main():
    ...
    speedup_vs_linear_cpu = (
        baseline["search_avg_latency_ms"] / cpu_results["search_avg_latency_ms"]
        if cpu_results["search_avg_latency_ms"] > 0
        else 0
    )
    speedup_vs_linear_vulkan = (
        baseline["search_avg_latency_ms"] / vulkan_results["search_avg_latency_ms"]
        if vulkan_results["search_avg_latency_ms"] > 0
        else 0
    )
    vulkan_vs_cpu = (
        cpu_results["search_avg_latency_ms"] / vulkan_results["search_avg_latency_ms"]
        if vulkan_results["search_avg_latency_ms"] > 0
        else 0
    )
    ...
    print("-" * 80)
    print(
        f"{'Avg Latency (ms)':<30} | {baseline['search_avg_latency_ms']:<15.4f} | {cpu_results['search_avg_latency_ms']:<15.4f} | {vulkan_results['search_avg_latency_ms']:<15.4f}"
    )
    print(
        f"{'P50 Latency (ms)':<30} | {baseline['search_p50_latency_ms']:<15.4f} | {cpu_results['search_p50_latency_ms']:<15.4f} | {vulkan_results['search_p50_latency_ms']:<15.4f}"
    )
    print(
        f"{'Throughput (QPS)':<30} | {baseline['search_throughput_qps']:<15.2f} | {cpu_results['search_throughput_qps']:<15.2f} | {vulkan_results['search_throughput_qps']:<15.2f}"
    )
    print(
        f"{'P95 Latency (ms)':<30} | {'N/A':<15} | {cpu_results['search_p95_latency_ms']:<15.4f} | {vulkan_results['search_p95_latency_ms']:<15.4f}"
    )
    print(
        f"{'Speedup vs Linear':<30} | {'1.0x':<15} | {speedup_vs_linear_cpu:<15.1f}x | {speedup_vs_linear_vulkan:<15.1f}x"
    )
    print(
        f"{'Vulkan GPU vs CPU':<30} | {'---':<15} | {'---':<15} | {vulkan_vs_cpu:<15.1f}x"
    )
    print("-" * 80)
