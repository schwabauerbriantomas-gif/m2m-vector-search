...
def benchmark_transformed(vectors: np.ndarray, queries: np.ndarray, k: int = 10):
    ...
    config_orig = M2MConfig(
        device="cpu", latent_dim=vectors.shape[1], max_splats=len(vectors) + 1000
    )
    ...
    transformer = M2MDatasetTransformer(
        vectors_norm, n_clusters_base=200, hierarchy_levels=4
    )
