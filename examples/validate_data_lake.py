...
def load_real_dataset(num_samples=10000, dim=640):
    try:
        ...
        tensor_emb = X.astype(np.float32)

        # Project real data to 640 dimensions to match the M2M spec
        if tensor_emb.shape[1] != dim:
            print(
                f"[INFO] Projecting {tensor_emb.shape[1]}D digit vectors to {dim}D M2M latent space..."
            )
            proj = np.random.randn(tensor_emb.shape[1], dim).astype(np.float32)
            tensor_emb = tensor_emb @ proj

        print(
            f"[SUCCESS] Loaded {len(tensor_emb)} real-world embeddings (Clusters: {len(digits.target_names)})"
        )
        return tensor_emb
    except Exception as e:
        print(f"[WARNING] Could not load scikit dataset, falling back: {e}")
        np.random.seed(42)
        clusters = np.random.randn(50, dim).astype(np.float32) * 5
        assignments = np.random.randint(0, 50, num_samples)
        data = (
            clusters[assignments]
            + np.random.randn(num_samples, dim).astype(np.float32) * 0.5
        )
        return data
...
def validate_data_lake(device="cpu", enable_vulkan=False):
    ...
    dataset_size = 10000
    mock_embeddings = load_real_dataset(num_samples=dataset_size, dim=config.latent_dim)
    ...
    print(
        f"Ingested {dataset_size} splats in {ingest_time:.2f}s ({(dataset_size/ingest_time):.2f} splats/sec)"
    )
    ...
    for batch_idx, batch_mu in enumerate(dataloader):
        # Simulated backwards propagation
        loss = np.random.rand() * 1.5
        total_loss_std += loss
        batches += 1
    ...
    print(
        f"Standard Epoch completed in {epoch_time:.2f}s | Avg Loss: {avg_loss_std:.4f}"
    )

    metrics["standard_training"]["epoch_time_seconds"] = epoch_time
    metrics["standard_training"]["avg_loss"] = avg_loss_std
    metrics["standard_training"]["throughput_splats_per_sec"] = dataset_size / max(
        epoch_time, 0.001
    )
    ...
    print(
        f"Generative Epoch completed in {epoch_time_gen:.2f}s | Avg Loss: {avg_loss_gen:.4f}"
    )

    metrics["generative_training"]["epoch_time_seconds"] = epoch_time_gen
    metrics["generative_training"]["avg_loss"] = avg_loss_gen
    metrics["generative_training"]["throughput_splats_per_sec"] = dataset_size / max(
        epoch_time_gen, 0.001
    )
    ...
    # Warmup
    m2m.search(queries[0][np.newaxis, :], k=10)
