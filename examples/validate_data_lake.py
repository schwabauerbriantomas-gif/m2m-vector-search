...
def load_real_dataset(num_samples=10000, dim=640):
    try:
        ...
        data = clusters[assignments] + np.random.randn(num_samples, dim).astype(np.float32) * 0.5
        return data
...
def validate_data_lake(device='cpu', enable_vulkan=False):
    ...
    metrics["standard_training"]["throughput_splats_per_sec"] = dataset_size / max(epoch_time, 0.001)
    ...
    metrics["generative_training"]["throughput_splats_per_sec"] = dataset_size / max(epoch_time_gen, 0.001)
